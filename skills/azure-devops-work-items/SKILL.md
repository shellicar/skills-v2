...
description: |
  ...
  TRIGGER WHEN: always, creating or updating any Azure DevOps work item.
---
...
Query the parent first (for its area/iteration path), create the item, then parent it
immediately — an unparented item is an orphaned one, hard to find later. Ask if the
parent isn't clear rather than guessing.

```
az boards work-item relation add --id <ID> --relation-type parent --target-id <PARENT_ID>
```

`--fields "System.Parent=<ID>"` on `update` silently does nothing — parenting only
works through `relation add`/`relation remove`, never the field.
